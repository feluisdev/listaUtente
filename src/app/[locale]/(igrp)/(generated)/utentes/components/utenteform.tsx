...
import { useFetchPessoaByNif } from '@/app/[locale]/(myapp)/functions/services/utente-service';
import { useFetchPessoaByIdentificacao } from '@/app/[locale]/(myapp)/functions/services/utente-service';
...
export default function Utenteform({ isEdit, utente }: { isEdit?: boolean; utente?: any }) {
  ...
  const [selecttipoIdentificacaoOptions, setSelecttipoIdentificacaoOptions] = useState<
    IGRPOptionsProps[]
  >([]);
  ...
  const { data: dataIdentificacao, isLoading: isLoadingIdentificacao } =
    useFetchPessoaByIdentificacao({
      tipoIdentificacao: tipoIdentificacaoValue,
      identificacao: searchIdentificacao,
    });
  ...
  useEffect(() => {
    if (formform1Ref.current) {
      const subscription = formform1Ref.current.watch((value) => {
        ...
        if (value.identificacao?.length && value.identificacao?.length > 4)
          setSearchIdentificacao(value.identificacao || '');
      });
