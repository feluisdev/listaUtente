...
function NovoUtenteContent() {
  ...
  const [tipoIdentificacaoOptions, setTipoIdentificacaoOptions] = useState<IGRPOptionsProps[]>([]);
  const [generoOptions, setGeneroOptions] = useState<IGRPOptionsProps[]>([]);
  ...
  useEffect(() => {
    ...
    const loadFormData = async () => {
      setLoading(true);
      try {
        // Carregar opções dos comboboxes
        const tipoUtenteOptions = getTipoUtente();
        setSelectcombobox1Options(tipoUtenteOptions);

        // Carregar opções do tipo de identificação e gênero
        const { getTipoDocumento, getGenero } = await import('@/app/(myapp)/functions/services/config-service');
        const tipoDocOptions = getTipoDocumento();
        setTipoIdentificacaoOptions(tipoDocOptions);
        
        const generoOpts = getGenero();
        setGeneroOptions(generoOpts);
        setTipoIdentificacaoOptions(tipoDocOptions);
